...
\problem{2}{Induction}
Induction becomes so much easier with \LaTeX! Have a look!

Define \pn as $n > n - 1$.

We will now show that \pn holds for all $n \in \N^+$.

\BC $n = 1$

\px{1} holds because $1 > 0$.

\IH Assume that \pk holds for arbitrary $k \geq 1$.
...
Hence, \pk is true. Since \pk implies \pkpo and \px{1} is true, by the principle of mathematical induction \pn holds for $n \in \N^+$.
